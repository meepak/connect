...
import {
  getDefaultColors, /* mergeJsonObjects, sleep */
  sleep,
...
const AppLoader = () => {
  ...
  const hideSplashScreen = async () => {
    ...
    // await sleep(300)
    // console.log('hidhing splash...')
    await SplashScreen.hideAsync()
  }

  // Incase app got ready before splash screen needed to load, this will unblock the closure of splash screen
  useEffect(() => {
    if (appDataPreloaded && authUser.status !== AuthStatus.Checking) {
      // console.log('app is ready to be displayed, hiding splash screen')
      hideSplashScreen()
    }
  }, [appDataPreloaded, authUser.status])
  ...
  const updateUserData = (newUserData: any) => {
      ...
      if (asset.localUri !== null) {
        ...
        Promise.resolve(
          asset
            .downloadAsync()
            .then((ass) => {
              setFunction(ass.localUri)
            })
            .catch((err) =>
              console.log(fieldName, `${newValue} -- ${err.message}`),
            ),
        )
      }
    }
    ...
    if (updatedData?.avatar) {
      updateField('avatar', updatedData.avatar, (item: any) => {
        updatedData.avatar = item
      })
    }
    if (updatedData?.bannerImage) {
      updateField('bannerImage', updatedData.bannerImage, (item: any) => {
        updatedData.bannerImage = item
      })
    }
  ...
  const handleSnapshot = (querySnapshot) => {
    ...
    const requireUpdate =
      JSON.stringify(updatedData) !== JSON.stringify(authUser.data)
    if (requireUpdate) {

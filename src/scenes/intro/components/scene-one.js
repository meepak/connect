...
const LOGO_INITIAL_X = ((width - LOGO_FINAL_DIMENSION - NAME_WIDTH) / 2)
const LOGO_INITIAL_Y = CENTER_STAGE_HEIGHT / 4.0

const LOGO_FINAL_X = 0 // To be adjusted
const LOGO_FINAL_Y = 0 // To be adjusted

// const NAME_INITIAL_X = width

const NAME_INITIAL_X = LOGO_FINAL_X + LOGO_FINAL_DIMENSION + 10 // width - LOGO_FINAL_DIMENSION - LOGO_FINAL_X
...
const NAME_FINAL_Y = 10 // To be adjusted

const TEXT_OPACITY_INITIAL = 0.65
const TEXT_OPACITY_FINAL = 1
...
const IntroSceneOne = ({
  onLoaded, onFinished, color, bgColor, replay,
}) => {
  ...
  const textY = useSharedValue(NAME_INITIAL_Y)
  const textOpacity = useSharedValue(TEXT_OPACITY_INITIAL)

  React.useEffect(() => {
    // Use a flag to determine whether to play the animation or reset to initial state
    if (replay) {
      // Replay animation
      ...
      textY.value = withTiming(NAME_FINAL_Y, { duration: DURATION, easing: Easing.inOut(Easing.ease) }, onFinished && onFinished(true))
      textOpacity.value = withTiming(TEXT_OPACITY_FINAL, { duration: DURATION, easing: Easing.inOut(Easing.linear) })
    } else {
      // Reset to initial state
      logoX.value = LOGO_INITIAL_X
      logoY.value = LOGO_INITIAL_Y
      logoDimension.value = LOGO_INITIAL_DIMENSION

      textX.value = NAME_INITIAL_X
      textY.value = NAME_INITIAL_Y
      textOpacity.value = TEXT_OPACITY_INITIAL
    }
  }, [replay])
  ...
  return (
    <View style={{ ...styles.container, backgroundColor: bgColor }} onLayout={() => onLoaded && onLoaded(true)}>
      ...
      >
        ...
        <Animated.View style={{
          ...styles.text, left: textX, top: textY, opacity: textOpacity,
        }}
        >
          <SvgName color={color} />
        </Animated.View>
...
IntroSceneOne.propTypes = {
  ...
  replay: PropTypes.bool, // Prop to trigger the replay
}

IntroSceneOne.defaultProps = {
  onLoaded: null,
  onFinished: null,
  replay: false, // Default to false
}

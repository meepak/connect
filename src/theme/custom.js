...
) => themeFromSourceColor(argbFromHex(color)).schemes[type].toJSON()

const argbThemeToHexTheme = (theme/*: ARGBTheme */) => Object.fromEntries(
  // @ts-ignore
  Object.entries(theme).map(([key, value]) => [key, Color(value).hex()]),
) // as CSSTheme;

const argbThemeToRgbTheme = (theme/*: ARGBTheme */) => Object.fromEntries(
  // @ts-ignore
...
export const hexThemeFromColor = (
  color, /*: string, */
  type, /*: 'light' | 'dark' = 'light' */
) => argbThemeToHexTheme(argbThemeFromColor(color, type))
...
const prepareElevations = (argbTheme/*: ARGBTheme */) => {
  ...
  for (let i = 0; i < elevationLevels.length; i += 1) {
